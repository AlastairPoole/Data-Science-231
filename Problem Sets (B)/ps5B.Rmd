...
> ANSWER: Kriti Verman, Brandon Kwon
...
> ANSWER: There seems to be a much higher concentration of U.S. Supreme Court Justices that have served on the court for a tenure of less than 20 years. This is when compared to Justices that have served for at least 25 years. This could be because you need to have a considerable amount of legal experience before assuming a seat, and as a result most appointed justices are elderly. So despite having their seat "for life" many might reach a retirement age relatively early in their career serving on the Supreme Court. 

```{r}
justices <- read_csv("~/Desktop/Data-Science-231/justices.csv")
...
ggplot(data = justices2, aes(x = tenure_length)) +
  geom_density(fill = "seagreen", color = "midnightblue") +
  labs(title = "Distribution of U.S. Supreme Court Justice's Term Lengths",
       x = "Tenure Length (years)")
```
...
```{r, linewidth = 60}
quotes_dat1 <- read_csv("~/Desktop/Data-Science-231/quotes.csv")

quote_for_the_day1 <- quotes_dat1[sample(1:nrow(quotes_dat1), size = 3),]

cat(quote_for_the_day1$together)
```
