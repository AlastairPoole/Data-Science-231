...
> ANSWER:Brandon Kwon and TA hours with Andrea!
...
> ANSWER: 10 pitchers meet this critera.

```{r}
Pitching_well <- Pitching %>%
  group_by(playerID) %>%
  summarize(totalwins = sum(W), totalSO = sum(SO)) %>%
...
> ANSWER: Ryan is the pitcher with the most accumulated strikeouts (his player ID is ryanno01). He had accumulated 5714 strikeouts. In 1973, he struck out the most hitters, and he had 383 strikeouts that season.
...
> ANSWER: It looks like there is a weak positive association between the number of inspections and the median score. It also appears as though institutions with very few inspections also have incredibly low median scores. But then as the number of inspections increases, the median score at these insitutions approaches roughly 25. 

```{r}
Manhattan_violations <- Violations %>%
...
ggplot(data = Manhattan_violations) +
...
```{r}
#Filters to find the result with the highest median score of violations
#This point  is that top left outlier on the graph
Manhattan_violations %>%
  arrange(desc(med_score)) %>%
  head(n=4)

#Same code as above to graph the plot
ggplot(data = Manhattan_violations) +
  geom_point(aes(x = num_inspections, y = med_score)) +
  
  #Adds Curved Arrow
  geom_curve(
  aes(x = 65, y = 50, xend = 10, yend = 68),
  data = Manhattan_violations,
  curvature = 0.2,
