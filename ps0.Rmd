...
This file has 1846 rows and three columns. The variables included are datasets, x-values, and y-values.
...
> RESPONSE: 
 ...
 The correlation coefficient is -0.06447185. This means that the relationship between x and y in the dino dataset is incredibly weak- to the point where there is almost no correlation.
```{r}
...
There does not really appear to be a relationship, but there is a cool sketch of T-Rex head!
...
The correlation coefficient is -0.0629611. This correlation is very similar (to the nearest hundredth place) to the coefficient from the dino dataset.
...
This plot is similar in that there is practically no correlation between x and y, however, it looks different because the pattern is a star instead of a T-Rex head.
...
A similarly weak correlation in which the coefficient is similar to the hundredths decimal place again.
...
This plot now looks like a circle instead of a star or a T-Rex head.
...
These correlation coefficients all demonstrate incredibly weak correlations between x and y in any of the datasets. This is expected because we are using scatterplots to make patterns that look a certain way, and none of these patterns include a linear correlation. This means that none of these correlation coefficients are appropriate summaries for any of these datasets.
