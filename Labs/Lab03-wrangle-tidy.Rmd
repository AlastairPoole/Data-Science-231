...
```{r}
coverage <- read_csv("/Users/alastairpoole/Desktop/Data-Science-231/Labs/healthcare_coverage.txt")
spending <- read_csv("/Users/alastairpoole/Desktop/Data-Science-231/Labs/healthcare_spending.txt")
```
...
> RESPONSE: The coverage dataset covers the years 2013-2016, and the spending dataset covers the years 1991-2014. So the only overlap is between 2013-2014, which is what we are confirming to be true.

```{r}
coverage1314 <- coverage %>% 
  select(Location, starts_with(c("2013", "2014")))
spending1314 <- spending %>% 
  select(Location, starts_with(c("2013", "2014")))
coverage1314
spending1314
```
...
```{r}
states_coverage1314 <- coverage1314 %>%
  filter(Location != "United States" & Location != "District of Columbia")
states_spending1314 <- spending1314 %>%
  filter(Location != "United States" & Location != "District of Columbia")
states_coverage1314
states_spending1314
```
...
> RESPONSE: The variables needed for the x-axis will be "2013_Total Health Spending" and "2014_Total Health Spending". The variables for the y-axis will be "2013_Employer" and "2014_Employer". We will also need the variable "Location". Each observation in the dataset will be a point on the scatterplot representing a particular state's healthcare spending and employer covereage for a particular year.
